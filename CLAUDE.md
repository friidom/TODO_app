...
**Positions.** Both todos and columns carry a dense integer `position`. Reordering means: recompute every affected position client-side, write the whole array into the query cache, then bulk `upsert` it (`reorderTodos` / `reorderColumns`). Never assume a partial update is enough — gaps in `position` break the sort. `byPosition`, the comparator both features sort with, lives in `src/utils/position.ts` — it is pure and shared, so it belongs to neither feature folder.
