...
Requires `.env` with `VITE_API_URL` (gitignored; `.env.example` is the template). `src/services/api/client.ts` throws at module load if it is missing. `VITE_API_URL` is now the **only** variable the frontend reads: realtime moved to the API's Socket.IO server (B9), attachments and avatars to MinIO (B10), and `src/services/api/supabase.ts` went with the last of them. Vite inlines it at build time, so a missing value is a rebuild, not a redeploy.
...
`supabase/migrations/` is the **historical** schema from before B5–B8 and is not applied by anything. B9 and B10 have since been built — realtime on Socket.IO, storage on MinIO — so it is now a record of where the schema came from rather than a source of rules still to port. Do not add to it.
...
**`column_id` and `sprint_id` are separate columns answering separate questions, and only `isOnBoard` may read them together.** `column_id` answers *is this on a column?*; `sprint_id` answers *is this planned into a Sprint?*. `isOnBoard` (`services/todos/backlog.ts`) is the one place board membership is decided, and the Board is **the running sprint's work and nothing else**: a card needs a column *and* must carry the active sprint's id. Unplanned work, a future sprint's work and a completed sprint's carried-over work are all off it, and with no active sprint the board is empty by design — `KanbanBoard`'s "No active sprint" empty state is what stops that reading as a bug. **`isOnBoard` also takes `sprintsEnabled`**: with Sprints off (0020) a column is the whole rule again, and without that branch turning the feature off would empty every board rather than simplify it. This was the opposite rule until the sprint-visibility fix; an earlier pass at the same change (M31-C) was reverted because it emptied every board with no explanation, so **the empty state is load-bearing, not decoration** — do not remove it while this rule stands. Everything else still keeps the two axes apart: `buildBacklogBoard` sections by `sprint_id` alone and `sprintAssignmentPatch` writes them independently, so folding the pair together anywhere else re-creates the trap. `useAddTodo` stamps the active sprint itself so no create surface has to remember to — which also means a card created while no sprint is running is unplanned, and so is not on the board.
...
**OAuth, and why an email never authenticates (0023).** "Continue with Google" and "Continue with GitHub" are a new way to *arrive* at `issueSession()`, not a second auth system: the callback sets the same refresh cookie the password flow sets, redirects to `APP_URL`, and the frontend's existing `refreshSession() → fetchMe()` mount effect does the rest — **`AuthProvider`, `client.ts` and `session.ts` needed no change at all**, and no token ever goes into a URL. Flow: `GET /auth/oauth/:provider/start` mints state, PKCE and a nonce into one signed `oauth_tx` cookie and 302s to the provider; `GET /auth/oauth/:provider/callback` exchanges the code server-side and 302s back. Both live in `modules/auth/` rather than a module of their own, because the whole point is to reach that module's `issueSession`, `auth.cookies` and `PublicUser`.

**`(provider, provider_account_id)` is the only thing that authenticates.** `oauth_accounts` is keyed on it (UNIQUE), and `resolveIdentity` in `oauth.service.ts` — pure, I/O-free, pinned by `oauth.service.test.ts` the way `permissions.ts` and `workflow.ts` are — is the single place the rule lives. A known identity signs in **and its email is not consulted at all**, so a provider renaming an address cannot lock anyone out. An unknown identity whose *verified* email already belongs to an account is **not** signed in, **not** linked and **does not** create a second account: it is parked in `oauth_link_tokens` and the browser is sent to `/login?link=…`, because the existing account may have been pre-registered by an attacker who simply typed the victim's address — this app does not verify addresses at registration, so that attack is live. Signing in normally is the proof of ownership, and `confirmLink` refuses unless **the authenticated caller is the account the token names**; without that one check the endpoint becomes "link this identity to whoever clicks", which is a phishable takeover. Two identities pointing at two real accounts is a 409, never a merge — re-parenting boards, memberships and `board_members` across two ids can *collide*, and there is no correct automatic answer.

**What each provider is trusted for.** Google is OIDC: the ID token is verified against Google's JWKS with `aud`, `iss`, `exp` and the `nonce` all enforced (`lib/oauth/google.ts`; no library — Node parses the JWK and `jsonwebtoken` does the rest). GitHub is plain OAuth2 with no ID token, so `state` carries the whole CSRF burden and PKCE is skipped because its OAuth App web flow does not document it. **GitHub's `/user.email` is never read** — it is the public profile field, free text and often unverified, and trusting it is the classic takeover bug; the address comes from `/user/emails` where `primary && verified`. The identity key is GitHub's numeric `id`, **never `login`**, which is renameable and re-claimable. Both adapters normalize to one `ProviderIdentity` so `oauth.service.ts` never learns which provider it is handling, and **both discard the provider's access token**: nothing calls a provider API afterwards, so `oauth_accounts` deliberately has no token columns and a breach here is not a breach of the user's Google account.

**Two things are load-bearing and easy to undo by accident.** The `oauth_tx` cookie pins `sameSite: "lax"` *explicitly* rather than reading `COOKIE_SAMESITE`: it must survive the provider's cross-site top-level redirect, and a Strict cookie is not sent on one — `COOKIE_SAMESITE=strict` would break every sign-in as "invalid state". And it carries a `typ` claim checked on read, because it is signed with the same `JWT_SECRET` and HS256 as access tokens and `verifyAccessToken` accepts any HS256 token with `sub` and `jti` — adding a `subject` to this payload would otherwise turn an HttpOnly cookie into a valid bearer token. `users.password_hash` is nullable now (NULL means "OAuth-only"), and `login()` still runs `verifyDummyPassword` on that branch: returning early would make "this address signs in with Google" measurable. `lib/redact.ts` strips `code`/`state` from the access log and the error handler, because morgan prints `:url` and writes through stdout where `verifyAuth`'s console capture could never see it.
...
**Column categories are the status model.** `columns.category` is a checked text field (`'todo' | 'in_progress' | 'in_review' | 'done'`) — a fixed set users pick from, never define, so there is no lookup table. There is no `todos.status`: a card's status IS the category of the column it sits in, which is why `todos.completed` could be dropped in M2. The palette lives in `src/constants/columns.ts`, not the DB: colours are presentation, so retuning them is an edit rather than a migration. `categoryOf()` falls back to `todo`, so a row with a null category still renders.

`in_review` arrived in **0019** and was not cosmetic. "In Review" shipped as a *second* `in_progress` column, so it was not a stage: `To Do → In Review` read as one step and was allowed, and `In Progress → Done` could not be refused at all. The backfill is deliberately narrow — it matches only the title this app itself seeds, because titles are user-editable free text. **Adding a fifth value means auditing every reader that buckets by category**: `summary.ts`, `sprintPoints.ts`, `timelineDrag.ts`, `admin.service.ts`'s `SYSTEM_WIP`, `WipStrip.tsx` and the three i18n files each assume the set they were written against. The two `todos` triggers do not — 0013 keys on `= 'done'` and 0017 on `<> 'todo'`, so a new middle stage is automatically "started, not done".

**The sequential workflow (0019/0020).** `backend/src/lib/workflow.ts` is the rule — stages in order, a move forward of one step, backward and sideways free, so only *skipping* is refused. It is enforced in exactly one place, `requireAllowedTransition` in `todos.service.ts`, which both status-writing paths (`POST /todos/:todoId/move`, `PATCH /boards/:boardId/todos/:todoId`) already call; adding a third write of `column_id` means calling it there too. A board is only held to **the stages it actually has** — refusing `in_progress → done` on a board with no In Review column would leave work unable to reach Done at all. `src/services/todos/workflow.ts` is the UI's mirror, kept honest by `workflow-matrix.json` and a parity test in each package, exactly as `permissions.ts` is; it decides what to *offer*, never what is allowed.

**Board Settings (0020).** `boards.sprints_enabled` and `boards.workflow_enabled`, both `not null default true`, patched through the board's existing admin-gated `PATCH /boards/:boardId` rather than a settings route of its own. They live on the **board**, not the space — `docs/ARCHITECTURE.md`'s "Does this belong to a Board?", and `IMPLEMENTATION_PLAN.md`'s "Per-board or global? Per-board". 0018 briefly put the workflow flag on `spaces`; 0021 backfilled it onto boards and 0022 dropped it, because a space is a private folder with an owner and no membership, so a board admin could not reach a rule that governed their board. Sprints OFF is reversible and deletes nothing: `isOnBoard` falls back to the pre-sprint rule and the sprint surfaces hide.
