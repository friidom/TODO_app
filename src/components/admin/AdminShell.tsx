          ...
          >
            {adminSections().map((section) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "text-ink-3 hover:text-ink hover:bg-wash rounded-control text-meta shrink-0 px-2.5 py-1.5 transition-colors",
                    isActive &&
                      "bg-brand-soft text-brand hover:bg-brand/20 hover:text-brand",
                  )
                }
              >
                {section.label}
