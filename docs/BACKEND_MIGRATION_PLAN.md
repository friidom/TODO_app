...
## B8 — Frontend migration from Supabase to Express ✅ DONE
...
**Actual result.** Every data path runs on the Express API. `@supabase/supabase-js` is still installed and `services/api/supabase.ts` still exists, because **realtime (B9) and storage (B10) were held out of scope** — see "What still imports Supabase" below. Frontend: 970 tests, typecheck, build and lint green. Backend: 144 unit, 321 integration, four probes unchanged.

**The one thing `tsc` cannot check is the wire**, so the whole migrated surface was replayed against a running server — the same paths and bodies the React code now sends, asserting the shapes it destructures. 58 checks, all passing, covering register → refresh → me, profile, spaces, boards, columns, todos (including the upsert-PATCH), comments, activity, members, invites, sprints, notifications and all three feed reads.

### What changed beyond a like-for-like port

| Change | Why it was forced |
|---|---|
| `PATCH` on a todo is `/boards/:boardId/todos/:todoId` | §10.4 — the upsert cannot pass the strict child resolver |
| The invite credential is in the request **body** | an access log records URLs, and the token is a bearer credential |
| Accept/decline take `{ token }` **or** `{ invite_id }` | S3 hashes the token, so a listed invite has none |
| **A pending invite's link can no longer be re-copied** | same reason — the plaintext is returned once, at creation. `InvitePeopleModal` still offers it there; the per-row copy button in `PendingInviteRow` is gone |
| `reorderTodos` is **not** ported | there is no endpoint that takes a client array of ids and positions, and deliberately so — M3-10 closed that as unnecessary. The rank correction after a mid-column insert stays; the dense `position` mirror is left stale, and nothing orders by it while a rank is present |
| The account email moved from `useProfile()` to `useAuth()` | `GET /users/me` returns the public profile shape, which withholds `email`. The address belongs to the account, and `/auth/me` already carried it |
| `updatePassword(token, password)` | the API deliberately issues no session from a reset, so the link cannot double as a login. `ResetPasswordPage` reads `?token=` and its old session-exchange race and 4s timeout are gone |
| Two endpoints added to B7's feed | `GET /me/worked-on` (ids + dates) and `GET /me/todos?ids=`. The worked-on and viewed tabs date each card by when *you* touched it; returning rows alone would have silently re-ordered both tabs by `updated_at` |

### What still imports Supabase, and why

| File | Deferred to |
|---|---|
| `services/api/supabase.ts` | the client itself — it survives for the two below |
| `services/realtime/useBoardRealtime.ts` | **B9** |
| `services/attachments/attachmentsApi.ts` | **B10** |
| `services/profile/uploadAvatars.ts` | **B10** |

`types/database.ts` is generated by `supabase gen types` and is still the source of the row types. It is a build-time artifact with no runtime dependency, and the tables it describes are the same tables; B10 is the natural point to regenerate it from Prisma or hand it over.

**So B8-14 is only half done, deliberately.** The plan's "zero references to `supabase` under `src/`" assumed B9 and B10 landed inside B8. They did not, so the dependency, the two `VITE_SUPABASE_*` variables and that one client file stay until B10 closes. **No `VITE_API_MODE` flag was ever added**: it exists in the plan to let the 19 files land across separate commits, and this migration landed as one.

**Removed:** `username.live.test.ts` and `vitest.live.config.ts` with its `test:live` script — the only live suite tested the `username_available` RPC, which no longer exists. That check is now `GET /auth/username-available`, covered by the backend suite.
...
**Verification.** ☑ `npm run build` and `npm test` green ☑ ~~zero references to `supabase`~~ — four files remain, each named above and owned by B9 or B10 ☑ every path the client builds checked against the mounted route table ☑ the whole surface replayed against a running server (58 checks) ☐ **B8-16's manual pass is still outstanding** — it needs a person and a browser ☐ **two tabs with an expired access token**, confirming one refresh and neither signed out.

**The last two cannot be automated here and are the real remaining risk.** The single-flight refresh is built to the letter of B8-01a — one in-flight promise per tab, a `navigator.locks` mutex across tabs, and a `BroadcastChannel` that hands the new token to whoever was waiting — but its failure mode is *unreproducible logouts*, and only two real tabs prove it.
...
> ### **B9 — Realtime using WebSocket / Socket.IO**
>
> B8 is complete: every data path in the React app runs on the Express API, verified by 970 frontend tests, a route-table audit and 58 live checks against a running server.
>
> B9 replaces Supabase Realtime with Socket.IO. `useBoardRealtime` keeps its signature (§13.4) — it is the last thing holding the Supabase client in the frontend apart from storage.
>
> **Two things B8 leaves for a person, not a test:** §16's B8-16 manual pass, and opening the app in two tabs with an expired access token to confirm one refresh is sent and neither tab is signed out. The single-flight refresh is built to B8-01a's letter, but its failure mode is unreproducible logouts and only two real tabs prove it.
>
> **§21.9 now blocks B10**, which is what finally removes `@supabase/supabase-js`.
...
**End of plan.** B0–B8 are built: the app runs on its own backend. B9 (realtime) and B10 (storage) are what still hold the Supabase client in the frontend, and B10 is what finally removes the dependency. 21.9 blocks B10 and B12; 21.10's premise was found to be stale in B7-0 and now needs verifying rather than deciding. One new item is open and belongs to nobody yet: `prune_activities` was never ported, so `activities` has no retention.
